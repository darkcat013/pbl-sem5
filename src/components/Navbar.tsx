import { AppBar, Box, CssBaseline, Divider, Drawer, Grid, IconButton, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Toolbar, Typography, Button } from '@mui/material';
...
const Navbar:React.FC<{setLoggedIn: Dispatch<React.SetStateAction<Boolean>>}> = ({setLoggedIn}) => {
    ...
    <Box sx={{ bgcolor: '#72A276', height: '100%' }}>
      <Toolbar >
        ...
        {mobileOpen ? menuButton :
          <IconButton onClick={() => navigate('/profile')} sx={{ width: "100%", mt: '30px', mb: '30px' }}>
      ...
      <List >
        ...
        {localStorage.getItem(localStorageKeys.role) != roles.student &&
        ...
        >
          <Toolbar>
            ...
            <Grid container direction={"row"} justifyContent={"right"} alignItems={"center"} sx={{ width: '100%' }} columnSpacing={4}>
              ...
              <Grid item >
                <Button variant="contained" color="primary" onClick={() => {localStorage.removeItem(localStorageKeys.role); setLoggedIn(false)}}>
                  Logout
                </Button>
              </Grid>
            </Grid>
